...
NodeOrEdge = CommentLine* WS* n:Node
{
  return {
    node: n
  }
}
/ CommentLine* WS* e:Edge
{
  return {
    edge: e
  }
...
Node = id:Value l:Label* p:Property* WS* InlineComment? EndOfLine
{
  let propObj = {};
  p.forEach(prop => {
    if (propObj[prop.key]) {
...
Edge = f:Value WS+ d:Direction WS+ t:Value l:Label* p:Property* WS* InlineComment? EndOfLine
{
  let propObj = {};
  p.forEach(prop => {
    if (propObj[prop.key]) {
