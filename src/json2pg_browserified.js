      ...
      function (require, module, exports) {
        ...
        function labelLength(element) {
          ...
          return element.labels
            .map((l) => l.quoteIfNeeded().length + 3)
        ...
        function propLength(element, prop) {
          ...
          return element.properties[prop]
            .map((v) => propNameLength + value.toString().quoteIfNeeded() + 3)
            .reduce((sum, l) => sum + l, 0);
        }
        ...
        function createNodeContents(pgObject, alignColumns) {
          ...
          if (alignColumns) {
            for (let node of pgObject.nodes) {
              maxIdLength = Math.max(
                maxIdLength,
                node.id.quoteIfNeeded().length
              );
              // Add three for ':' and trailing two spaces
              maxLabelLength = Math.max(maxLabelLength, labelLength(node));
              for (let property in node.properties) {
                maxPropLength[property] = Math.max(
                  maxPropLength[property],
                  propLength(node, property)
                );
              }
            }
          }
          ...
          for (let node of pgObject.nodes) {
            ...
            let nodeContents = [
              node.id.quoteIfNeeded().padEnd(maxIdLength * 2 - idPart.length),
            ];
            let labelPart = "";
            ...
            if (alignColumns) {
              ...
              for (let property in maxPropLength) {
                for (let value of node.properties[property]) {
                  propPart +=
                    (
                      property.quoteIfNeeded() +
                      ":" +
                      value.toString().quoteIfNeeded()
                    ).padEnd(maxPropLength[property]) + "  ";
                }
              }
        ...
        function createEdgeContents(pgObject, alignColumns) {
          ...
          if (alignColumns) {
            for (let edge of pgObject.edges) {
              maxFromLength = Math.max(
                maxFromLength,
                edge.from.quoteIfNeeded().length
              );
              maxToLength = Math.max(
                maxToLength,
                edge.to.quoteIfNeeded().length
              );
              maxLabelLength = Math.max(maxLabelLength, labelLength(edge));
              for (let property in edge.properties) {
                maxPropLength[property] = Math.max(
                  maxPropLength[property],
                  propLength(edge, property)
                );
              }
            }
          }
        ...
        exports.translate = (json, alignColumns = false) => {
          let pgObject;
          if (typeof json == "string") pgObject = JSON.parse(json);
          else pgObject = json;
          return (
            createNodeContents(pgObject, alignColumns) +
            createEdgeContents(pgObject, alignColumns)
          );
        };
      },
      {},
